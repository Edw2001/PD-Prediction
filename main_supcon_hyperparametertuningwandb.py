...
import wandb
wandb.login(key="f888a7726dcc66f3fce9bdf833952a3721ca21f4")
...
def parse_option():
    ...
    parser.add_argument('--wandb_project', type=str, default='supcon_project', help='WandB project name')

    opt = parser.parse_args()

    # check if dataset is path that passed required arguments
    if opt.dataset == 'path':
        assert opt.data_folder is not None, "Data folder must be provided for 'path' dataset"
...
def train():
    ...
    # Parse options using WandB configurations
    opt = parse_option()

    # Setup data loaders, model, optimizer based on wandb config
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        wandb.log({"epoch": epoch, "loss": losses.avg})
...
if __name__ == '__main__':
    main()
    sweep_id = wandb.sweep(sweep_config, project="SupConTraining")
